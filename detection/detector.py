...
JUNCTION_ID = 1
SEND_INTERVAL = 3       # seconds between API updates
...
def estimate_speed(track_id, current_center, fps=30):
    """Estimate vehicle speed based on pixel movement between frames"""
    if track_id in previous_positions:
        ...
        # Rough conversion: assume 1 pixel = 0.05 meters at typical camera height
        speed_ms = pixel_distance * 0.05 * fps
        speed_kmh = speed_ms * 3.6
        previous_positions[track_id] = current_center
        return round(min(speed_kmh, 120), 1)  # Cap at 120 km/h
    else:
        previous_positions[track_id] = current_center
        return 0
...
def calculate_congestion_score(total_vehicles, avg_speed):
    ...
    0   = free flow
    100 = complete gridlock
    """
    vehicle_score = min(total_vehicles * 4, 60)   # Max 60 points from vehicle count
    speed_score = max(0, 40 - avg_speed * 0.8)    # Max 40 points from low speed
...
def send_to_api(cars, motorcycles, buses, trucks, pedestrians, speeds, congestion_score):
    """Send detection data to TrafficBrain API"""
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Sent — "
                  f"Cars:{cars} Bikes:{motorcycles} Buses:{buses} Trucks:{trucks} "
                  f"Peds:{pedestrians} Speed:{avg_speed}km/h Congestion:{congestion_score}/100")
        else:
    ...
    while True:
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                # Label with name, confidence, speed
        ...
        # ── HUD overlay ──
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (260, 220), (0, 0, 0), -1)
